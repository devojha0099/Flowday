...
const StreakBadge = ({ streak }) => {
  // Support both old format (type) and new format (category)
  const type = streak.type || streak.category;
  const meta = STREAK_META[type] || {
    label: streak.category || "Streak",
    icon: streak.icon || "🔥",
    color: "indigo",
  };
  ...
  return (
    <div
      className={`rounded-xl border ${c.bg} ${c.border} p-4 flex flex-col items-center text-center`}
    >
