...
const LiveTimer = ({ plan }) => {
  ...
  const [form, setForm] = useState({ title: '', category: 'Study', plannedBlockId: '' })
  ...
  const handleStop = () => {
    if (activeTimer?.id) dispatch(stopTimer(activeTimer.id))
  }
  ...
  return (
    <div className="space-y-3">
      <div className={`rounded-lg p-4 text-center border ${activeTimer ? 'bg-green-50 border-green-200' : 'bg-slate-50 border-slate-200'}`}>
        <div className={`text-3xl font-bold font-mono tracking-widest ${activeTimer ? 'text-green-600' : 'text-slate-400'}`}>
          {activeTimer ? elapsed : '00:00:00'}
        </div>
        {activeTimer
          ? <p className="text-xs text-green-600 mt-1 truncate">Tracking: {activeTimer.title}</p>
      ...
      {activeTimer ? (
        ...
        <div className="space-y-2">
          <input type="text" placeholder="What are you working on?" value={form.title}
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <select value={form.plannedBlockId}
              onChange={e => {
          ...
          <button onClick={handleStart} disabled={isLoading || !form.title.trim()}
