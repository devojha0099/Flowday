      ...
      <div className="inline-block min-w-full">
              ...
              <div key={wi} style={{ width: '13px', flexShrink: 0 }}>
                {monthEntry && (
                  <span className="text-[9px] text-slate-400">
                    {MONTH_LABELS[Number(monthEntry[0])]}
                  </span>
                )}
              </div>
            )
        ...
        <div className="flex gap-0">
          ...
          <div className="flex flex-col mr-1">
            {DAY_LABELS.map((label, i) => (
              <div
                key={i}
                className="text-[9px] text-slate-400 flex items-center justify-center"
              ...
              <div key={wi} className="flex flex-col gap-[2px]">
                {week.map((day, di) => {
                  ...
                  return (
                    <div
                      key={di}
                      title={`${dateStr}: ${score > 0 ? score + '/100' : 'No data'}`}
                      style={{
                        width: '11px',
                        height: '11px',
                        borderRadius: '2px',
        ...
        <div className="flex items-center gap-1.5 mt-2 justify-end">
          ...
          {['#f1f5f9', '#c7d2fe', '#818cf8', '#6366f1', '#4338ca'].map((color) => (
            <div
              key={color}
              style={{ width: '11px', height: '11px', borderRadius: '2px', backgroundColor: color }}
            />
          ))}
