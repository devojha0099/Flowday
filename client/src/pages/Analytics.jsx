...
const StatCard = ({ label, value, sub, valueColor }) => (
  <div className="bg-white rounded-xl border border-slate-200 p-4">
    ...
    <p className={`text-2xl font-bold ${valueColor || "text-slate-800"}`}>
      {value}
    </p>
...
const Analytics = () => {
  ...
  const { weekly, monthly, heatmap, isLoading } = useSelector(
  ...
  const weeklyStats = weekly?.days
    ? {
        avgScore: weekly.days.filter((d) => d.score > 0).length
          ? Math.round(
              weekly.days
                .filter((d) => d.score > 0)
                .reduce((s, d) => s + d.score, 0) /
                weekly.days.filter((d) => d.score > 0).length,
            )
          : 0,
                ...
                {weekly.days.map((day) => (
                  ...
                  >
                    ...
                    <span className="text-xs text-slate-400 text-right">
                      {fmtMins(day.actualMins)}
                    </span>
                    <span className="text-xs text-slate-300 text-right">
                      {fmtMins(day.plannedMins)}
                    </span>
                    <span
                      className={`text-xs font-semibold text-right ${day.score >= 70 ? "text-green-600" : day.score >= 40 ? "text-amber-500" : "text-red-400"}`}
                    >
          ...
          {monthly ? (
            <>
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <StatCard
                  label="Active days"
                  value={monthly.totalDays}
                  sub="Days with data"
                />
                <StatCard
                  label="Total tracked"
                  value={fmtMins(monthly.totalActualMins)}
                  sub="This month"
                />
                <StatCard
                  label="Best day"
                  value={
                    monthly.bestDay ? `${monthly.bestDay.overallScore}%` : "—"
                  }
                  sub={
                    monthly.bestDay
                      ? new Date(
                          monthly.bestDay.date + "T12:00:00",
          ...
          {progress.length === 0 ? (
            <div className="bg-white rounded-xl border border-slate-200 p-8 text-center">
              ...
              <p className="text-xs text-slate-400">
                Go to the Goals page to add weekly targets
              </p>
            </div>
          ) : (
            <div className="bg-white rounded-xl border border-slate-200 p-6">
