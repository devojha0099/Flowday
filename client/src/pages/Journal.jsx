...
import { loadJournal, saveJournal, loadAllJournals, searchJournals, clearSearch } from '../features/journal/journalSlice'
import { useToday, formatDisplayDate } from '../hooks/useToday'
import Layout from '../components/layout/Layout'

const MOODS = [
  { value: 1, label: 'Rough',   emoji: '😞', color: 'bg-red-100 text-red-600 border-red-200' },
...
const Journal = () => {
  ...
  const { current, all, searchResults, isSaving } = useSelector(state => state.journal)
  ...
  return (
    <Layout>
      <div className="mb-6">
        ...
        <p className="text-slate-500 text-sm mt-0.5">Daily reflections and notes</p>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-5">
            <div className="flex items-center justify-between flex-wrap gap-4">
              ...
              <div>
                <label className="block text-xs text-slate-500 mb-1">How did today feel?</label>
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className={`text-xs font-medium px-3 py-1 rounded transition-colors ${
                    !preview ? 'bg-white text-indigo-600 shadow-sm border border-slate-200' : 'text-slate-400 hover:text-slate-600'
                  }`}
                >
                  Write
                </button>
            ...
            {!preview ? (
              ...
              <div className="px-5 py-4 prose prose-sm max-w-none min-h-[320px] text-slate-700">
                {content
                  ...
                  : <p className="text-slate-300 text-sm">Nothing to preview yet.</p>
                }
              </div>
            )}
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-4">
            <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-3">
              {searchResults ? `Results (${searchResults.length})` : 'Recent entries'}
            </h3>
                ...
                {displayList.map(entry => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left p-3 rounded-lg border transition-all hover:border-indigo-200 hover:bg-indigo-50 ${
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-medium text-slate-700">
                          {new Date(entry.date + 'T12:00').toLocaleDateString('en', {
